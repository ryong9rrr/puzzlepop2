...
type IconType = "timer" | "percent" | "image";
...
export const SideWidgetContainer = ({ children }: PropsWithChildren) => {
  return (
...
const renderIcon = (iconType: IconType) => {
  if (iconType === "timer") {
    return <IoTimer size={30} />;
  }
  if (iconType === "percent") {
    return <IoPercent size={24} />;
  }
  if (iconType === "image") {
    return <IoImage size={22} />;
  }
  return null;
};
