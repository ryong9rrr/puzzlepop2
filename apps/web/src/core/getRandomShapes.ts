...
// 차후에 API 붙이면 필요없어짐
export const getRandomShapes = ({ perColumn, perRow }: { perColumn: number; perRow: number }) => {
  ...
  for (let y = 0; y < perColumn; y++) {
    for (let x = 0; x < perRow; x++) {
      ...
      const shapeRight = x < perRow - 1 ? shapes[y * perRow + (x + 1)] : undefined;

      const shapeBottom = y < perColumn - 1 ? shapes[(y + 1) * perRow + x] : undefined;
